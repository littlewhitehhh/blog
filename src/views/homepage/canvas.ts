...
class Wave {
  ...
  constructor(e: Partial<Pick<Wave, 'phase' | 'amplitude' | 'frequency' | 'offset'>>) {
...
class Line {
  ...
  constructor(
    ...
    this.friction = E.friction + 0.01 * Math.random() - 0.05;
...
export const renderCanvas = () => {
  ...
  function animate() {
    if (running) {
      (ctx.globalCompositeOperation = 'source-over'), ctx.clearRect(0, 0, ctx.canvas.width, ctx.canvas.height);
